let playerHP = 100, energy = 3, playerShield = 0;
    ...
    function drawCards() {
      ...
      deck.forEach((card, i) => {
        const div = document.createElement("div");
        let tipo = card.name === "Ataque" || card.name === "Impacto Bruto" || card.name === "Fogo Amigo" || card.name === "Drenagem" || card.name === "Beserck" || card.name === "Explosão" || card.name === "Chuva De Fragmentos" || card.name === "Rajada Dupla" ? "attack" : card.name === "Recicladora" || card.name === "Ataque Reciclável" || card.name === "Defesa Reciclável" || card.name === "Xenofluxo Reciclável" || card.name === "Destruir Carta" ? "reciclagem" : card.name === "Entulho" || card.name === "Lobo" || card.name === "Alcateia" || card.name === "Chamado do Alfa" || card.name === "Arma Quebrada" || card.name === "Escudo quebrado" || card.name === "Restos de meca" || card.name === "Ferro Velho" ? "lixo" :
          card.name === "Defesa" || card.name === "Sistema de reflexão" || card.name === "Escudo Retaliante" || card.name === "Impulso Defensivo" || card.name === "PROTOCOL-Campo De Força" || card.name === "Brilhando" || card.name === "Escudo" ? "defense" : card.name === "GÆPROTOCOL" ? "cintilante" :
            card.name === "Cura" || card.name === "Mineração" || card.name === "PROTOCOL-Reforço Estrutural" || card.name === "Impulso" || card.name === "Golpe Neural Retaliante" || card.name === "Compra Dupla" || card.name === "Sobre Carga" ? "heal" : "";
